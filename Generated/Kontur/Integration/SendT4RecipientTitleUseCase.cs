/*
  ФАЙЛ: SendT4RecipientTitleUseCase.cs
  НАЗНАЧЕНИЕ: Сценарий отправки ответного титула T4 через явный gateway-порт Контур.
  Отделяет бизнес-шаг T4 от UI, сборки XML, импорта подписи и крупного KonturAdapter.

  ЖУРНАЛ ИЗМЕНЕНИЙ:
  29.05.2026 - Первичное создание сценария отправки ответного титула T4 в реконструкционном слое.
...
{
    /// <summary>
    /// Выполняет прикладной сценарий отправки ответного титула T4 и фиксирует явное состояние этапа.
    /// </summary>
    /// <remarks>Сценарий требует подтвержденный предыдущий этап и не переводит T4 в завершенное состояние автоматически.</remarks>
    public class SendT4RecipientTitleUseCase
    {
        /// <summary>
        /// Код этапа T4 в явном состоянии процесса.
        /// </summary>
        private const string StageCode = "T4";

        /// <summary>
        /// Код титула T4 в ЭТрН.
        /// </summary>
        private const string TitleCode = "T4";
        ...
        public SendT4RecipientTitleUseCase(
        ...
        /// <summary>
        /// Отправляет ответный титул T4 в существующий операторный документооборот и фиксирует результат в явном состоянии этапа.
        /// </summary>
        ...
        /// <returns>Результат отправки ответного титула T4.</returns>
        ...
        public KonturStageExecutionResult Execute(long timelineId, string senderBoxId)
        {
            ...
            if (transitionDecision == null || !transitionDecision.IsAllowed)
            {
                SaveFailedState(
                    ...
                    transitionDecision == null ? "Policy перехода не вернула решение для T4." : transitionDecision.Message);
            ...
            {
                SaveFailedState(timelineId, "TitleXmlMissing", "XML титула T4 не найден. Сначала нужно выполнить сборку XML этапа.");
                return Fail(timelineId, "TitleXmlMissing");
            }

            if (!artifact.HasSignature)
            {
                SaveFailedState(timelineId, "SignatureMissing", "Подпись титула T4 не импортирована. Сначала нужно выполнить шаг импорта подписи.");
            ...
            {
                SaveFailedState(timelineId, "TransportationIdMissing", "TransportationId не найден. T4 нельзя отправлять без связи с существующей перевозкой.");
            ...
            if (result == null)
            {
                SaveFailedState(timelineId, "SendReturnedNull", "Gateway отправки T4 вернул пустой результат.");
                return Fail(timelineId, "SendReturnedNull");
            }

            if (!result.IsSuccess)
            {
                SaveFailedState(timelineId, "SendT4Failed", result.Message);
                return result;
            }
        ...
        /// <summary>
        /// Сохраняет состояние успешной отправки ответного титула T4.
        /// </summary>
        ...
        /// <remarks>Метод намеренно не выставляет Completed, потому что отправка T4 не равна подтвержденному завершению этапа оператором.</remarks>
        private void SaveSentState(long timelineId, KonturStageExecutionResult result)
        {
            ...
            state.LastOperatorStatus = result == null || string.IsNullOrEmpty(result.Message) ? "T4Sent" : result.Message;
        ...
        /// <summary>
        /// Сохраняет состояние неуспешной отправки ответного титула T4.
        /// </summary>
        /// <param name="timelineId">Идентификатор timeline документа.</param>
        ...
        /// <summary>
        /// Формирует неуспешный результат отправки ответного титула T4.
        /// </summary>
        /// <param name="timelineId">Идентификатор timeline документа.</param>
        /// <param name="message">Техническое сообщение ошибки.</param>
        /// <returns>Неуспешный результат T4-сценария.</returns>
