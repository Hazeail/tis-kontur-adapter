/*
  ФАЙЛ: SendT2RecipientTitleUseCase.cs
  НАЗНАЧЕНИЕ: Сценарий отправки ответного титула T2 через явный gateway-порт Контур.
  Отделяет бизнес-шаг T2 от UI, сборки XML, импорта подписи и крупного KonturAdapter.

  ЖУРНАЛ ИЗМЕНЕНИЙ:
  29.05.2026 - Первичное создание сценария отправки ответного титула T2 в реконструкционном слое.
...
{
    /// <summary>
    /// Выполняет прикладной сценарий отправки ответного титула T2 и фиксирует явное состояние этапа.
    /// </summary>
    /// <remarks>Сценарий не моделирует T2 как повторный старт первого титула и требует подтвержденный предыдущий этап.</remarks>
    public class SendT2RecipientTitleUseCase
    {
        /// <summary>
        /// Код этапа T2 в явном состоянии процесса.
        /// </summary>
        private const string StageCode = "T2";

        /// <summary>
        /// Код титула T2 в ЭТрН.
        /// </summary>
        private const string TitleCode = "T2";
        ...
        public SendT2RecipientTitleUseCase(
        ...
        /// <summary>
        /// Отправляет ответный титул T2 в существующий операторный документооборот и фиксирует результат в явном состоянии этапа.
        /// </summary>
        ...
        /// <returns>Результат отправки ответного титула T2.</returns>
        ...
        public KonturStageExecutionResult Execute(long timelineId, string senderBoxId)
        {
            ...
            if (transitionDecision == null || !transitionDecision.IsAllowed)
            {
                SaveFailedState(
                    ...
                    transitionDecision == null ? "Policy перехода не вернула решение для T2." : transitionDecision.Message);
            ...
            {
                SaveFailedState(timelineId, "TitleXmlMissing", "XML титула T2 не найден. Сначала нужно выполнить сборку XML этапа.");
                return Fail(timelineId, "TitleXmlMissing");
            }

            if (!artifact.HasSignature)
            {
                SaveFailedState(timelineId, "SignatureMissing", "Подпись титула T2 не импортирована. Сначала нужно выполнить шаг импорта подписи.");
            ...
            {
                SaveFailedState(timelineId, "TransportationIdMissing", "TransportationId не найден. T2 нельзя отправлять как новый первый титул без связи с существующей перевозкой.");
            ...
            if (result == null)
            {
                SaveFailedState(timelineId, "SendReturnedNull", "Gateway отправки T2 вернул пустой результат.");
                return Fail(timelineId, "SendReturnedNull");
            }

            if (!result.IsSuccess)
            {
                SaveFailedState(timelineId, "SendT2Failed", result.Message);
                return result;
            }
        ...
        /// <summary>
        /// Сохраняет состояние успешной отправки ответного титула T2.
        /// </summary>
        ...
        /// <remarks>Метод намеренно не выставляет Completed, потому что отправка T2 не равна подтвержденному завершению этапа оператором.</remarks>
        private void SaveSentState(long timelineId, KonturStageExecutionResult result)
        {
            ...
            state.LastOperatorStatus = result == null || string.IsNullOrEmpty(result.Message) ? "T2Sent" : result.Message;
        ...
        /// <summary>
        /// Сохраняет состояние неуспешной отправки ответного титула T2.
        /// </summary>
        /// <param name="timelineId">Идентификатор timeline документа.</param>
        ...
        /// <summary>
        /// Формирует неуспешный результат отправки ответного титула T2.
        /// </summary>
        /// <param name="timelineId">Идентификатор timeline документа.</param>
        /// <param name="message">Техническое сообщение ошибки.</param>
        /// <returns>Неуспешный результат T2-сценария.</returns>
